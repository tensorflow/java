 ...
 */
public final class DebugIdentityV2<T extends TType> extends PrimitiveOp implements Operand<T> {
  
  /**
   * Optional attributes for {@link org.tensorflow.op.core.DebugIdentityV2}
   */
  ...
  public static <T extends TType> DebugIdentityV2<T> create(Scope scope, Operand<T> input, Options... options) {
    ...
    return new DebugIdentityV2<T>(opBuilder.build());
  }
  ...
  private DebugIdentityV2(Operation operation) {
    super(operation);
    int outputIdx = 0;
