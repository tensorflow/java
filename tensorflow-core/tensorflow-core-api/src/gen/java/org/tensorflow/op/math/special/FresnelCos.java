 ...
 */
@OpMetadata(
    opType = FresnelCos.OP_NAME,
    inputsClass = FresnelCos.Inputs.class
)
public final class FresnelCos<T extends TNumber> extends RawOp implements Operand<T> {
  ...
  public FresnelCos(Operation operation) {
  ...
  public static <T extends TNumber> FresnelCos<T> create(Scope scope, Operand<T> x) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "FresnelCos");
    opBuilder.addInput(x.asOutput());
    return new FresnelCos<>(opBuilder.build());
  }
  ...
  @OpInputsMetadata(
      outputsClass = FresnelCos.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<FresnelCos<T>> {
    /**
     * The x input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new FresnelCos<>(op), op, Arrays.asList("T"));
      int inputIndex = 0;
