 ...
 */
public class Average<T extends TFloating> extends Merge<T> {
   ...
   */
  public Average(Class<T> type, Options options) {
   ...
   * @param type the data type for the weights and computation
   */
  public Average(String name, Class<T> type) {
    this(name, type, null);
  }
   ...
   * @param options the layer's options
   */
  public Average(String name, Class<T> type, Options options) {
  ...
  protected Operand<? extends TType> mergeFunction(List<Operand<? extends TNumber>> inputs) {
    ...
    return tf.math.div(output, cast(tf, tf.constant(inputs.size()), getType()));
  }
}
