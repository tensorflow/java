...
public interface FloatDataLayout<S extends DataBuffer<?>> extends DataLayout<S, Float> {
  ...
  /**
   * Writes a float into the buffer at the given index after converting it to the raw data type.
   *
   ...
   * @see #writeValue(DataBuffer, Float, long)
   */
  void writeFloat(S buffer, float value, long index);

  /**
   * Reads one or more raw values from the buffer at the given index and to be returned as a float.
   *
  ...
  @Override
  default void writeValue(S buffer, Float value, long index) {
    writeFloat(buffer, value, index);
  }

  @Override
  default Float readValue(S buffer, long index) {
