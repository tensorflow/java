 ...
 */
public final class QuantizedConv2DAndRequantize<V extends TType> extends PrimitiveOp {
  
  /**
   * Optional attributes for {@link org.tensorflow.op.core.QuantizedConv2DAndRequantize}
   */
   ...
   */
  public static <V extends TType, T extends TType, U extends TType> QuantizedConv2DAndRequantize<V> create(Scope scope, Operand<T> input, Operand<U> filter, Operand<TFloat> minInput, Operand<TFloat> maxInput, Operand<TFloat> minFilter, Operand<TFloat> maxFilter, Operand<TFloat> minFreezedOutput, Operand<TFloat> maxFreezedOutput, DataType<V> outType, List<Long> strides, String padding, Options... options) {
    ...
    return new QuantizedConv2DAndRequantize<V>(opBuilder.build());
  }
  ...
  private QuantizedConv2DAndRequantize(Operation operation) {
    super(operation);
    int outputIdx = 0;
