...
public class Dropout<T extends TFloating> extends Layer<T> {
   ...
   */
  public Dropout(Ops tf, float rate, long seed, Class<T> type) {

    this(tf, null, rate, null, seed, type, null);
  }
   ...
   */
  public Dropout(Ops tf, float rate, long seed, Class<T> type, Options options) {
   ...
   * @param type the data type for the weights and computation
   */
  public Dropout(Ops tf, float rate, Shape noiseShape, long seed, Class<T> type) {
    this(tf, null, rate, noiseShape, seed, type, null);
  }
   ...
   * @param options the layer's options.
   */
  public Dropout(Ops tf, float rate, Shape noiseShape, long seed, Class<T> type, Options options) {
  ...
  public <U extends TType> List<Operand<U>> call(
    ...
    for (Operand<? extends TType> input : inputs) {
      ...
      if (!TFloating.class.isAssignableFrom(input.type())) {
        output = cast(tf, input, TFloat64.class);
      } else {
