...
public class SparseOps(
     ...
     *  An ``` N```-minibatch of ``` SparseTensor``` objects is represented as a ```
     * SparseTensor```
     ...
     *  ``` result_values```, and ``` result_shape```. For ``` set1``` and ``` set2``` ranked ```
     * n```, this
     *  has rank ``` n``` and the same 1st ``` n-1``` dimensions as ``` set1``` and ``` set2```. The
     * ``` nth```
     ...
     *  See SetOperationOp::SetOperationFromContext for values of ``` set_operation```.
     *  Input ``` set2``` is a ``` SparseTensor``` represented by ``` set2_indices```, ```
     * set2_values```,
     *  and ``` set2_shape```. For ``` set2``` ranked ``` n```, 1st ``` n-1``` dimensions must be
     * the same
     ...
     *  (``` indices```, ``` values```, ``` dense_shape```).  The output ``` SparseTensor``` has
     * the
     ...
     *  If the input tensor has rank ``` R_in``` and ``` N``` non-empty values, and ```
     * new_shape```
     *  has length ``` R_out```, then ``` input_indices``` has shape ``` [N, R_in]```,
     *  ``` input_shape``` has length ``` R_in```, ``` output_indices``` has shape ``` [N,
     * R_out]```, and
     ...
     *  missing, the ``` output``` tensor at that position will be zeroed.
     *  Read
     *   <a href="https://tensorflow.org/api_docs/python/tf/sparse#Segmentation">the section on
     * segmentation</a>
     *  for an explanation of segments.
     *  For example:
     *
     ...
     *  logical submatrix with shape ``` [B, C]```, but with the catch that <em>the implicitly
     ...
     *  See SetOperationOp::SetOperationFromContext for values of ``` set_operation```.
     *  If ``` validate_indices``` is ``` True```, ``` sparse.SparseToSparseSetOperation```
     * validates the
     *  order and range of ``` set1``` and ``` set2``` indices.
     *  Input ``` set1``` is a ``` SparseTensor``` represented by ``` set1_indices```, ```
     * set1_values```,
     *  and ``` set1_shape```. For ``` set1``` ranked ``` n```, 1st ``` n-1``` dimensions must be
     * the same
    ...
    public inline fun <reified T : TType> sparseAccumulatorTakeGradient(
        handle: Operand<TString>,
        numRequired: Operand<TInt32>
    ...
    public inline fun <reified T : TType> sparseConditionalAccumulator(
        ...
        sharedName: String? = null,
        reductionType: String? = null
    ): SparseConditionalAccumulator = sparseConditionalAccumulator<T>(
        T::class.java, shape,
    ...
    public inline fun <reified T : TType> takeManySparseFromTensorsMap(
        ...
        container: String? = null,
        sharedName: String? = null
    ): TakeManySparseFromTensorsMap<T> = takeManySparseFromTensorsMap<T>(
        sparseHandles,
