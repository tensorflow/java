...
public final class Ceil<T extends TNumber> extends RawOp implements Operand<T> {
  ...
  public Ceil(Operation operation) {
  ...
  public static <T extends TNumber> Ceil<T> create(Scope scope, Operand<T> x) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "Ceil");
  ...
  @OpInputsMetadata(
      outputsClass = Ceil.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<Ceil<T>> {
    /**
     * The x input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new Ceil<>(op), op, Arrays.asList("T"));
      int inputIndex = 0;
