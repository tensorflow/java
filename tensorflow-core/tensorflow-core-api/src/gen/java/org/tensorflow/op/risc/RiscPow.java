 ...
 */
@OpMetadata(
    opType = RiscPow.OP_NAME,
    inputsClass = RiscPow.Inputs.class
)
public final class RiscPow<T extends TNumber> extends RawOp implements Operand<T> {
  ...
  public RiscPow(Operation operation) {
  ...
  @OpInputsMetadata(
      outputsClass = RiscPow.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<RiscPow<T>> {
    /**
     * The x input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new RiscPow<>(op), op, Arrays.asList("T"));
      int inputIndex = 0;
