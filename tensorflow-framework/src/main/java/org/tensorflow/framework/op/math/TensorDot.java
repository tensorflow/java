...
// NOTE: This is implemented as tf.tensordot() in Python, and the code is in
// python/ops/math_ops.py. Is this where we want to keep this?
/**
 * Implements the TensorDot (Tensor contraction of a and b along specified axes and outer product)
 * operation.
 */
@Operator(group = "math")
public abstract  class TensorDot {
   ...
   *     None, representing the inferred static shape of the free dimensions
   */
  private static <T extends TNumber> Object[] tensordotReshape(
    ...
    if (!aShape.hasUnknownDimension()) { // calculate using static values
