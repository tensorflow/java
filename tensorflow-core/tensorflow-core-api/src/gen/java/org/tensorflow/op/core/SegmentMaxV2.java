 ...
 */
@OpMetadata(
    opType = SegmentMaxV2.OP_NAME,
    inputsClass = SegmentMaxV2.Inputs.class
)
@Operator
public final class SegmentMaxV2<T extends TNumber> extends RawOp implements Operand<T> {
  /**
  ...
  public SegmentMaxV2(Operation operation) {
  ...
  public static <T extends TNumber> SegmentMaxV2<T> create(Scope scope, Operand<T> data,
      Operand<? extends TNumber> segmentIds, Operand<? extends TNumber> numSegments) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "SegmentMaxV2");
    opBuilder.addInput(data.asOutput());
    opBuilder.addInput(segmentIds.asOutput());
    opBuilder.addInput(numSegments.asOutput());
    return new SegmentMaxV2<>(opBuilder.build());
  }
  ...
  @OpInputsMetadata(
      outputsClass = SegmentMaxV2.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<SegmentMaxV2<T>> {
    /**
     * The data input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new SegmentMaxV2<>(op), op, Arrays.asList("T", "Tindices", "Tnumsegments"));
      int inputIndex = 0;
