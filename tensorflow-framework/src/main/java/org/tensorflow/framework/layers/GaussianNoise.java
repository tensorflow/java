...
public class GaussianNoise<T extends TFloating> extends Layer<T> {
   ...
   */
  public GaussianNoise(float stddev, long seed, Class<T> type) {

    this(null, stddev, seed, type, null);
  }
   ...
   */
  public GaussianNoise(float stddev, long seed, Class<T> type, Options options) {

    this(null, stddev, seed, type, options);
  }
   ...
   * @param type the data type for the weights and computation
   */
  public GaussianNoise(String name, float stddev, long seed, Class<T> type) {
    this(name, stddev, seed, type, null);
  }
  /**
   * Creates a GaussianNoise layer
   *
   ...
   * @param options the layer's options.
   */
  public GaussianNoise(String name, float stddev, long seed, Class<T> type, Options options) {
  ...
  public <U extends TType> List<Operand<U>> call(
    ...
    for (Operand<? extends TType> input : inputs) {
      ...
      Operand<T> output = cast(ltf, input, getType());

      if (training) {

        Operand<T> stddevV = cast(ltf, ltf.constant(stddev), getType());
