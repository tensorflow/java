...
@OpMetadata(
    ...
    inputsClass = Identity.Inputs.class
)
@Operator
public final class Identity<T extends TType> extends RawOp implements Operand<T> {
  ...
  public Identity(Operation operation) {
  ...
  public static <T extends TType> Identity<T> create(Scope scope, Operand<T> input) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "Identity");
  ...
  @OpInputsMetadata(
      outputsClass = Identity.class
  )
  public static class Inputs<T extends TType> extends RawOpInputs<Identity<T>> {
    /**
     * The input input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new Identity<>(op), op, Arrays.asList("T"));
      int inputIndex = 0;
