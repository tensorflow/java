 ...
 */
@Operator
public final class XlaVariadicReduceV2 extends RawOp implements Iterable<Operand<TType>> {
  /**
  ...
  private XlaVariadicReduceV2(Operation operation) {
    super(operation);
    int outputIdx = 0;
  ...
  public static XlaVariadicReduceV2 create(Scope scope, Iterable<Operand<?>> inputs,
      Iterable<Operand<?>> initValues, List<Long> dimensionsToReduce, ConcreteFunction reducer) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "XlaVariadicReduceV2");
    ...
    return new XlaVariadicReduceV2(opBuilder.build());
  }
  ...
  public static class Inputs extends RawOpInputs<XlaVariadicReduceV2> {
    /**
     * the input tensor(s)
     */
    ...
    public Inputs(GraphOperation op) {
      super(new XlaVariadicReduceV2(op), op, Arrays.asList("T", "dimensions_to_reduce"));
      int inputIndex = 0;
