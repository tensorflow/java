 ...
 */
@OpMetadata(
    opType = RefEnter.OP_NAME,
    inputsClass = RefEnter.Inputs.class
)
public final class RefEnter<T extends TType> extends RawOp implements Operand<T> {
  ...
  public static <T extends TType> RefEnter<T> create(Scope scope, Operand<T> data, String frameName,
      Options... options) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "RefEnter");
  ...
  public static class Inputs<T extends TType> extends RawOpInputs<RefEnter<T>> {
    ...
    public Inputs(GraphOperation op) {
      super(new RefEnter<>(op), op, Arrays.asList("T", "frame_name", "is_constant", "parallel_iterations"));
      int inputIndex = 0;
