...
public interface NdArray<T> extends Shaped {
   ...
   * {@link #size()} values.
   *
   * <p>For example, given a {@code n x m} matrix on the {@code [x, y]} axes, elements are iterated
   ...
   *
   * <p>The returned sequence can then be iterated to visit each elements, either by calling {@link
   ...
   *
   * <ul>
   *   <li>new shape is known (i.e. has no unknown dimension)
   *   <li>new shape size is equal to the size of the current shape (i.e. same number of elements)
   * </ul>
   *
   * For example,
   *
   ...
   * }</pre>
   *
   * <p>Any changes applied to the returned view affect the data of this array as well, as there is
   * no copy involved.
   *
   ...
   *
   * <p>Any changes applied to the returned slice affect the data of this array as well, as there is
   * no copy involved.
   *
   * <p>Example of usage:
   *
   ...
   *
   * <p>The size of the buffer must be equal or greater to the {@link #size()} of this array, or an
   * exception is thrown. After the copy, content of the buffer and of the array can be altered
   * independently, without affecting each other.
   *
   * <p><i>Note: in version 0.4.0 and earlier, this method was named {@code read(DataBuffer<T>)}. It
   * has been renamed to explicitly indicate the direction of the data flow to avoid confusion.</i>
   *
   ...
   * independently, without affecting each other.
   *
   * <p><i>Note: in version 0.4.0 and earlier, this method was named {@code write(DataBuffer<T>)}.
   * It has been renamed to explicitly indicate the direction of the data flow to avoid
   * confusion.</i>
   *
   ...
   *
   * <p>An array is equal to another object if this object is another {@link NdArray} of the same
   ...
   * }</pre>
   *
   * <p>Note that the computation required to verify equality between two arrays can be expensive in
