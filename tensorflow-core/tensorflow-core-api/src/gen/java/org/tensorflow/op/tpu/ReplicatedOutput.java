...
public final class ReplicatedOutput<T extends TType> extends RawOp implements Iterable<Operand<T>> {
  ...
  /**
   * Factory method to create a class wrapping a new ReplicatedOutput operation.
   * 
  ...
  public static <T extends TType> ReplicatedOutput<T> create(Scope scope, Operand<T> input, Long numReplicas) {
    OperationBuilder opBuilder = scope.env().opBuilder("TPUReplicatedOutput", scope.makeOpName("ReplicatedOutput"));
  ...
  private ReplicatedOutput(Operation operation) {
    super(operation);
    int outputIdx = 0;
