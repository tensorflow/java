...
@OpMetadata(
    opType = Prod.OP_NAME,
    inputsClass = Prod.Inputs.class
)
@Operator
public final class Prod<T extends TType> extends RawOp implements Operand<T> {
  /**
  ...
  public Prod(Operation operation) {
  ...
  public static <T extends TType> Prod<T> create(Scope scope, Operand<T> input,
      Operand<? extends TNumber> axis, Options... options) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "Prod");
    ...
    return new Prod<>(opBuilder.build());
  }
  ...
  @OpInputsMetadata(
      outputsClass = Prod.class
  )
  public static class Inputs<T extends TType> extends RawOpInputs<Prod<T>> {
    /**
    ...
    public Inputs(GraphOperation op) {
      super(new Prod<>(op), op, Arrays.asList("keep_dims", "T", "Tidx"));
      int inputIndex = 0;
