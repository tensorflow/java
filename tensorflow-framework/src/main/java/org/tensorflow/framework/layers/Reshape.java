...
public class Reshape<T extends TFloating> extends Layer<T> {
   ...
   */
  public Reshape(Shape targetShape, Class<T> type) {
    this(null, targetShape, type, null);
  }
   ...
   */
  public Reshape(Shape targetShape, Class<T> type, Options options) {
    this(null, targetShape, type, options);
  }
   ...
   * @param type the data type for the layer's weights and computation.
   */
  public Reshape(String name, Shape targetShape, Class<T> type) {
    this(name, targetShape, type, null);
  }
   ...
   * @param options the layer's options.
   */
  public Reshape(String name, Shape targetShape, Class<T> type, Options options) {
  ...
  public <U extends TType> List<Operand<U>> call(
    ...
    Operand<TInt64> newShapeOp = ltf.constant(newShape);
    inputs.forEach(inp -> result.add(ltf.reshape(cast(ltf, inp, getType()), newShapeOp)));
