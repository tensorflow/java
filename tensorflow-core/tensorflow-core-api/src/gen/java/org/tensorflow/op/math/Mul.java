...
public final class Mul<T extends TType> extends RawOp implements Operand<T> {
  ...
  public Mul(Operation operation) {
  ...
  public static <T extends TType> Mul<T> create(Scope scope, Operand<T> x, Operand<T> y) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "Mul");
    opBuilder.addInput(x.asOutput());
    opBuilder.addInput(y.asOutput());
    return new Mul<>(opBuilder.build());
  }
  ...
  @OpInputsMetadata(
      outputsClass = Mul.class
  )
  public static class Inputs<T extends TType> extends RawOpInputs<Mul<T>> {
    /**
     * The x input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new Mul<>(op), op, Arrays.asList("T"));
      int inputIndex = 0;
