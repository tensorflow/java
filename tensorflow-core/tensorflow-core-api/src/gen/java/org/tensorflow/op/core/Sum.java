...
@Operator
public final class Sum<T extends TType> extends RawOp implements Operand<T> {
  /**
  ...
  public static <T extends TType> Sum<T> create(Scope scope, Operand<T> input,
    ...
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "Sum");
    ...
    return new Sum<>(opBuilder.build());
  }
  ...
  public static class Inputs<T extends TType> extends RawOpInputs<Sum<T>> {
    /**
    ...
    public Inputs(GraphOperation op) {
      super(new Sum<>(op), op, Arrays.asList("keep_dims", "T", "Tidx"));
      int inputIndex = 0;
