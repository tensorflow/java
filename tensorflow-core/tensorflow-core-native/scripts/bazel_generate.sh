...
${BAZEL_CMD:=bazel} --bazelrc=tensorflow.bazelrc $BUILD_FLAGS ${BUILD_USER_FLAGS:-} \
    @org_tensorflow//tensorflow/tools/lib_package:libtensorflow_proto :java_proto_gen_sources
...
cd $GEN_SRCS_DIR
find $TENSORFLOW_BIN $BAZEL_BIN/external/tsl/tsl $BAZEL_BIN/external/xla/xla -name \*-speed-src.jar -exec jar xf {} \;
rm -rf META-INF
