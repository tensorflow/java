 ...
 */
@OpMetadata(
    opType = ScatterMin.OP_NAME,
    inputsClass = ScatterMin.Inputs.class
)
@Operator
public final class ScatterMin<T extends TNumber> extends RawOp implements Operand<T> {
  ...
  public ScatterMin(Operation operation) {
  ...
  public static <T extends TNumber> ScatterMin<T> create(Scope scope, Operand<T> ref,
    ...
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "ScatterMin");
  ...
  @OpInputsMetadata(
      outputsClass = ScatterMin.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<ScatterMin<T>> {
    /**
    ...
    public Inputs(GraphOperation op) {
      super(new ScatterMin<>(op), op, Arrays.asList("T", "Tindices", "use_locking"));
      int inputIndex = 0;
