 ...
 *
 * <ul>
 *   <li>64-bits indexing, allowing to work with buffer larger than 2<sup>31</sup> bytes
 *   <li>Storage of object of any types and not only primitives
...
public interface DataBuffer<T> {
   ...
   *
   * <p>For exemple, in case of a byte buffer, this value is equal to the number of bytes this
   ...
   *
   * <p><b>Important: </b>Usage of this method should be limited to buffers of non-primitive types
   ...
   *
   * <p>This method transfers values from this buffer into the given destination array. If there are
   ...
   *
   * <p>Otherwise, this method copies {@code n = dst.length} values from this buffer into the given
   * array.
   *
   ...
   *
   * <p>Otherwise, this method copies {@code n = length} values from this buffer into the given
   ...
   *
   * <p>This method transfers the values in the given source array into this buffer. If there are
   ...
   *
   * <p>Otherwise, this method copies {@code n = src.length} values from the given array.
   *
   ...
   *
   * <p>Otherwise, this method copies {@code n = length} values from the given array into this
   ...
   * Write the references of the objects in the source array into this buffer.
   *
   * <p>If there are more values to copy than the destination buffer size, i.e. {@code size >
   ...
   *
   * <p>Otherwise, this method copies {@code n = size} values from this buffer into the destination
   * buffer.
   *
   ...
   * at the given index.
   *
   * <p>The index must not be greater than this buffer size. Changes to this buffer's content will
   * be visible in the new buffer and vice versa. The new buffer will be read-only if, and only if,
   * this buffer is read-only.
   *
   * <p>This call is equivalent to {@link #slice(long, long) slice(index, size() - index)}
   *
  ...
  /**
   * Creates a new buffer whose content is a shared subsequence of this buffer's content, whose size
   * is set to the given value.
   *
   * <p>The new size must not be greater than this buffer size. Changes to this buffer's content
   * will be visible in the new buffer and vice versa. The new buffer will be read-only if, and only
   * if, this buffer is read-only.
   *
   * <p>This call is equivalent to {@link #slice(long, long) slice(0, size)}
   *
   ...
   *
   * <p>The index plus the new size must not be greater than this buffer size. Changes to this
   ...
   * storage to the provided visitor. The visitor does not have to handle all possible types of data
   * storage and can override only methods for storage it is actually interested in. For any other
   * type of storage, this call will fallback to {@link DataStorageVisitor#fallback()} so the
   ...
   * in some cases and therefore, it is recommended to not use this method in a critical path where
   * performances matter.
   *
