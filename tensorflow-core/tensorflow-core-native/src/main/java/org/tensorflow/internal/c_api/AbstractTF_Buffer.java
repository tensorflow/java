...
public abstract class AbstractTF_Buffer extends Pointer {
  protected static class DeleteDeallocator extends TF_Buffer implements Pointer.Deallocator {
    ...
    public void deallocate() {
      if (!isNull()) TF_DeleteBuffer(this);
      setNull();
    }
  }
  ...
  /**
   * Returns {@code newBufferFromString(new BytePointer(proto.toByteArray()))}, or null if proto is
   * null or empty.
   */
   ...
   *
   * @return TF_Buffer created, or null if proto is null or empty. Do not call TF_DeleteBuffer() on
   *     it.
   */
  ...
  public byte[] copyData() {
    ...
    if (length > Integer.MAX_VALUE) {
      throw new IndexOutOfBoundsException(
          "TF_Buffer is too large to serialize into a byte[] array");
    }
  ...
  /**
   * Returns the data of this buffer as a {@link ByteBuffer}
   *
   * @throws IndexOutOfBoundsException if too large.
   */
  public ByteBuffer dataAsByteBuffer() {
    ...
    if (length > Integer.MAX_VALUE) {
      throw new IndexOutOfBoundsException(
          "TF_Buffer is too large to accessed via a ByteBuffer interface");
    }
