...
@OpMetadata(
    ...
    inputsClass = Polygamma.Inputs.class
)
@Operator(
    group = "math"
)
public final class Polygamma<T extends TNumber> extends RawOp implements Operand<T> {
  ...
  public Polygamma(Operation operation) {
  ...
  public static <T extends TNumber> Polygamma<T> create(Scope scope, Operand<T> a, Operand<T> x) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "Polygamma");
  ...
  @OpInputsMetadata(
      outputsClass = Polygamma.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<Polygamma<T>> {
    /**
     * The a input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new Polygamma<>(op), op, Arrays.asList("T"));
      int inputIndex = 0;
