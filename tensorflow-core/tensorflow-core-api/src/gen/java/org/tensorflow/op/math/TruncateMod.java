...
@OpMetadata(
    ...
    inputsClass = TruncateMod.Inputs.class
)
@Operator(
    group = "math"
)
public final class TruncateMod<T extends TNumber> extends RawOp implements Operand<T> {
  ...
  public TruncateMod(Operation operation) {
  ...
  public static <T extends TNumber> TruncateMod<T> create(Scope scope, Operand<T> x, Operand<T> y) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "TruncateMod");
  ...
  @OpInputsMetadata(
      outputsClass = TruncateMod.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<TruncateMod<T>> {
    /**
     * The x input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new TruncateMod<>(op), op, Arrays.asList("T"));
      int inputIndex = 0;
