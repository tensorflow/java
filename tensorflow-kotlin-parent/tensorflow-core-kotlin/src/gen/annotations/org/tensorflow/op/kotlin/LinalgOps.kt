...
public class LinalgOps(
     ...
     *  Assume ``` input``` has ``` k``` dimensions ``` [I, J, K, ..., M, N]```, then the output is
     * a
     ...
     *  <strong>Note</strong>: The gradient computation on GPU is faster for large matrices but
     ...
     *  The output ``` y``` has the same rank as ``` x```. The shapes of ``` x``` and ``` y```
     * satisfy:
     ...
     *  Note: This Op is <em>not</em> intended to be called by the user; instead users should
     ...
     *  Similar to  <a
     * href="https://docs.scipy.org/doc/numpy/reference/generated/numpy.einsum.html">```
     * numpy.einsum```</a> .
     ...
     *  <ul>
     *  <li>``` row_remapping``` must have exactly ``` num_rows``` entries. Row ``` i``` of the
     * output
     ...
     *  <li>A value of -1 in either of the remappings signifies a &quot;missing&quot; entry. In
     * that
     ...
     *  The ``` log_abs_determinant``` is computed as ``` det(P)*sum(log(diag(LU)))``` where ```
     * LU```
     ...
     *  true).
     *  <em>Note</em>: The default kernel implementation for MatMul on GPUs uses
     *  cublas.
     *
     * @param T data type for ` product` output
     * @param a the a value
     ...
     *
     *  output&#91;i, j, ..., l, m, n]
     *    = diagonal&#91;i, j, ..., l, diag_index, index_in_diag] ; if k&#91;0] &lt;= d &lt;=
     * k&#91;1]
     *      padding_value                                     ; otherwise
     *
     ...
     *      padding_value                 ; otherwise.
     *
     *  where ``` d = k[1] - m```, ``` y = max(-d, 0) - offset```, and ``` x = max(d, 0) -
     * offset```.
     ...
     *  ``` input``` has ``` r+1``` dimensions ``` [I, J, ..., L, M, N]```. When ``` k``` is scalar
     * or
     *  ``` k[0] == k[1]```, ``` diagonal``` has ``` r``` dimensions ``` [I, J, ..., L,
     * max_diag_len]```.
     ...
     *  problem \(X = \mathrm{argmin}_{Z \in \Re^{n \times k} ```
     *  ||A Z - B||_F^2 + \lambda ||Z||<em>F^2\).
     ...
     *  ``` P``` is the minimum of ``` M``` and ``` N```, the 2 inner-most dimmensions of ```
     * tensor```.
     *
     *  # a is a tensor.
     ...
     * is
     *  a tensor shape ``` [..., M, K]```.  If ``` adjoint``` is ``` False``` then each output
     * matrix
