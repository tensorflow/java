 ...
 */
@Operator
public final class MatrixSetDiagV3<T extends TType> extends RawOp implements Operand<T> {
  
  /**
   * Optional attributes for {@link org.tensorflow.op.core.MatrixSetDiagV3}
   */
  ...
  public static <T extends TType> MatrixSetDiagV3<T> create(Scope scope, Operand<T> input, Operand<T> diagonal, Operand<TInt32> k, Options... options) {
    OperationBuilder opBuilder = scope.env().opBuilder("MatrixSetDiagV3", scope.makeOpName("MatrixSetDiagV3"));
    ...
    return new MatrixSetDiagV3<T>(opBuilder.build());
  }
  ...
  private MatrixSetDiagV3(Operation operation) {
    super(operation);
    int outputIdx = 0;
