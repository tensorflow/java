 ...
 */
@OpMetadata(
    opType = TPUPartitionedInputV2.OP_NAME,
    inputsClass = TPUPartitionedInputV2.Inputs.class
)
public final class TPUPartitionedInputV2<T extends TType> extends RawOp implements Operand<T> {
  /**
  ...
  public TPUPartitionedInputV2(Operation operation) {
   ...
   * @return a new instance of TPUPartitionedInputV2
   */
  @Endpoint(
      describeByClass = true
  )
  public static <T extends TType> TPUPartitionedInputV2<T> create(Scope scope,
      Iterable<Operand<T>> inputs, List<Long> partitionDims, Options... options) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "TPUPartitionedInputV2");
    ...
    return new TPUPartitionedInputV2<>(opBuilder.build());
  }
  ...
  /**
   * Optional attributes for {@link org.tensorflow.op.core.TPUPartitionedInputV2}
   */
  ...
  @OpInputsMetadata(
      outputsClass = TPUPartitionedInputV2.class
  )
  public static class Inputs<T extends TType> extends RawOpInputs<TPUPartitionedInputV2<T>> {
    /**
    ...
    public Inputs(GraphOperation op) {
      super(new TPUPartitionedInputV2<>(op), op, Arrays.asList("T", "partition_dims", "is_packed"));
      int inputIndex = 0;
