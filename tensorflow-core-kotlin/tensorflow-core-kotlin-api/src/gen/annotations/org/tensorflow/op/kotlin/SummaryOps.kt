...
public class SummaryOps(
  ...
  public fun audioSummary(
    ...
    vararg options: AudioSummary.Options
  ): AudioSummary = java.audioSummary(tag, tensor, sampleRate, *options)

  public fun <T : TNumber> histogramSummary(tag: Operand<TString>, values: Operand<T>):
      HistogramSummary = java.histogramSummary<T>(tag, values)

  public fun <T : TNumber> imageSummary(
    tag: Operand<TString>,
    tensor: Operand<T>,
    vararg options: ImageSummary.Options
  ): ImageSummary = java.imageSummary<T>(tag, tensor, *options)

  public fun mergeSummary(inputs: Iterable<Operand<TString>>): MergeSummary =
      java.mergeSummary(inputs)

  public fun <T : TNumber> scalarSummary(tags: Operand<TString>, values: Operand<T>): ScalarSummary
      = java.scalarSummary<T>(tags, values)

  public fun <T : TType> tensorSummary(
    tag: Operand<TString>,
    tensor: Operand<T>,
    serializedSummaryMetadata: Operand<TString>
  ): TensorSummary = java.tensorSummary<T>(tag, tensor, serializedSummaryMetadata)
}
