 ...
 */
@OpMetadata(
    opType = SegmentProdV2.OP_NAME,
    inputsClass = SegmentProdV2.Inputs.class
)
@Operator
public final class SegmentProdV2<T extends TType> extends RawOp implements Operand<T> {
  /**
  ...
  public SegmentProdV2(Operation operation) {
  ...
  public static <T extends TType> SegmentProdV2<T> create(Scope scope, Operand<T> data,
      Operand<? extends TNumber> segmentIds, Operand<? extends TNumber> numSegments) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "SegmentProdV2");
    opBuilder.addInput(data.asOutput());
    opBuilder.addInput(segmentIds.asOutput());
    opBuilder.addInput(numSegments.asOutput());
    return new SegmentProdV2<>(opBuilder.build());
  }
  ...
  @OpInputsMetadata(
      outputsClass = SegmentProdV2.class
  )
  public static class Inputs<T extends TType> extends RawOpInputs<SegmentProdV2<T>> {
    /**
     * The data input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new SegmentProdV2<>(op), op, Arrays.asList("T", "Tindices", "Tnumsegments"));
      int inputIndex = 0;
