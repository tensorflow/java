...
typedef int (*TF_GradFuncAdapter)(TF_GraphId graphId, TF_Scope* scope, TF_Operation* operation, TF_Output* grad_inputs, int grad_inputs_length, TF_Output** grad_outputs);

/// Returns true if a gradient function has already be registered for operations of type `op_type`
TF_CAPI_EXPORT extern bool TF_HasGradient(const char* op_type);
...
TF_CAPI_EXPORT extern bool TF_RegisterCustomGradient(const char* op_type, TF_GradFuncAdapter custom_gradient_adapter);
