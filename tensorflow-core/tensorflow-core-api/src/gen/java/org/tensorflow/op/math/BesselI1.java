...
public final class BesselI1<T extends TNumber> extends RawOp implements Operand<T> {
  ...
  public BesselI1(Operation operation) {
  ...
  public static <T extends TNumber> BesselI1<T> create(Scope scope, Operand<T> x) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "BesselI1");
    opBuilder.addInput(x.asOutput());
    return new BesselI1<>(opBuilder.build());
  }
  ...
  @OpInputsMetadata(
      outputsClass = BesselI1.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<BesselI1<T>> {
    /**
     * The x input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new BesselI1<>(op), op, Arrays.asList("T"));
      int inputIndex = 0;
