...
class FlattenTest {
  ...
  public void testCall() {
    try (TestSession session = TestSession.createTestSession(tfMode)) {
      ...
      Flatten<TFloat32> layer = new Flatten<>(tf, TFloat32.class);
      Operand<TFloat32> output = layer.call(input, TFloat32.class);
  ...
  public void testCallChannelsFirst() {
    try (TestSession session = TestSession.createTestSession(tfMode)) {
      ...
      Flatten<TFloat32> layer = new Flatten<>(tf, TensorFormat.NCHW, TFloat32.class);
      Operand<TFloat32> output = layer.call(input, TFloat32.class);
