...
@OpMetadata(
    ...
    inputsClass = SegmentSum.Inputs.class
)
@Operator(
    group = "math"
)
public final class SegmentSum<T extends TType> extends RawOp implements Operand<T> {
  ...
  public SegmentSum(Operation operation) {
  ...
  public static <T extends TType> SegmentSum<T> create(Scope scope, Operand<T> data,
    ...
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "SegmentSum");
  ...
  @OpInputsMetadata(
      outputsClass = SegmentSum.class
  )
  public static class Inputs<T extends TType> extends RawOpInputs<SegmentSum<T>> {
    /**
     * The data input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new SegmentSum<>(op), op, Arrays.asList("T", "Tindices", "Tnumsegments"));
      int inputIndex = 0;
