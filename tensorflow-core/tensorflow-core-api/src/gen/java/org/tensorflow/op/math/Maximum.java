...
@OpMetadata(
    ...
    inputsClass = Maximum.Inputs.class
)
@Operator(
    group = "math"
)
public final class Maximum<T extends TNumber> extends RawOp implements Operand<T> {
  ...
  public Maximum(Operation operation) {
  ...
  @OpInputsMetadata(
      outputsClass = Maximum.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<Maximum<T>> {
    /**
     * The x input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new Maximum<>(op), op, Arrays.asList("T"));
      int inputIndex = 0;
