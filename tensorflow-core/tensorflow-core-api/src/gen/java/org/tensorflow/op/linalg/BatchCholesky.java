...
@OpMetadata(
    ...
    inputsClass = BatchCholesky.Inputs.class
)
@Operator(
    group = "linalg"
)
public final class BatchCholesky<T extends TNumber> extends RawOp implements Operand<T> {
  ...
  public BatchCholesky(Operation operation) {
  ...
  public static <T extends TNumber> BatchCholesky<T> create(Scope scope, Operand<T> input) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "BatchCholesky");
  ...
  @OpInputsMetadata(
      outputsClass = BatchCholesky.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<BatchCholesky<T>> {
    /**
     * The input input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new BatchCholesky<>(op), op, Arrays.asList("T"));
      int inputIndex = 0;
