...
public class ShapeUtils {
   ...
   */
  public static <T extends TInteger> Shape toShape(Scope scope, Operand<T> dims) {
   ...
   */
  public static <T extends TInteger> long[] getLongArray(Scope scope, Operand<T> dims) {
    if (scope.env().isEager()) {
      return getLongArray(dims.asTensor());
    }
    try (Session session = new Session((Graph)scope.env());
        T tensor = session.runner().fetch(dims).run().get(0)) {
   ...
   * @throws java.lang.IllegalArgumentException if the dims type is not an integer
   */
  public static <T extends TInteger> long[] getLongArray(T dims) {
