 ...
 */
@OpMetadata(
    opType = IsInf.OP_NAME,
    inputsClass = IsInf.Inputs.class
)
@Operator(
    group = "math"
)
public final class IsInf extends RawOp implements Operand<TBool> {
  ...
  public IsInf(Operation operation) {
  ...
  public static IsInf create(Scope scope, Operand<? extends TNumber> x) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "IsInf");
  ...
  @OpInputsMetadata(
      outputsClass = IsInf.class
  )
  public static class Inputs extends RawOpInputs<IsInf> {
    /**
     * The x input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new IsInf(op), op, Arrays.asList("T"));
      int inputIndex = 0;
