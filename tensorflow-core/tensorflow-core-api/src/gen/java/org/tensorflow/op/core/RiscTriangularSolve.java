...
public final class RiscTriangularSolve<T extends TNumber> extends RawOp implements Operand<T> {
  ...
  private RiscTriangularSolve(Operation operation) {
    super(operation);
    int outputIdx = 0;
  ...
  public static <T extends TNumber> RiscTriangularSolve<T> create(Scope scope, Operand<T> matrix,
    ...
    OperationBuilder opBuilder = scope.env().opBuilder("RiscTriangularSolve", scope.makeOpName("RiscTriangularSolve"));
  ...
  /**
   * Optional attributes for {@link org.tensorflow.op.core.RiscTriangularSolve}
   */
