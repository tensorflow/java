 ...
 */
public final class ReadVariableXlaSplitND<T extends TType> extends RawOp implements Iterable<Operand<T>> {
  /**
  ...
  private ReadVariableXlaSplitND(Operation operation) {
    super(operation);
    int outputIdx = 0;
   ...
   * @return a new instance of ReadVariableXlaSplitND
   */
  @Endpoint(
      describeByClass = true
  )
  public static <T extends TType> ReadVariableXlaSplitND<T> create(Scope scope,
      Operand<? extends TType> resource, Class<T> T, Long N, List<Long> numSplits,
      Options... options) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "ReadVariableXlaSplitND");
    ...
    return new ReadVariableXlaSplitND<>(opBuilder.build());
  }
  ...
  /**
   * Optional attributes for {@link org.tensorflow.op.rawops.ReadVariableXlaSplitND}
   */
  ...
  public static class Inputs extends RawOpInputs<ReadVariableXlaSplitND<?>> {
    /**
    ...
    public Inputs(GraphOperation op) {
      super(new ReadVariableXlaSplitND<>(op), op, Arrays.asList("T", "num_splits", "paddings"));
      int inputIndex = 0;
