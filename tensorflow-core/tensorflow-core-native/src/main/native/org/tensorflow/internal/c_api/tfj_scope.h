...
TF_CAPI_EXPORT extern TFJ_Scope TFJ_NewRootScope();
...
TF_CAPI_EXPORT extern TFJ_Scope TFJ_NewSubScope(const TFJ_Scope* scope, const char* child_scope_name);
...
TF_CAPI_EXPORT extern TFJ_Scope TFJ_NewScopeWithControlDependencies(const TFJ_Scope* scope, TF_Operation* control_deps, int control_deps_size);

/// Return a new scope. All ops created within the returned scope will have
/// the device field set to 'device'.
TF_CAPI_EXPORT extern TFJ_Scope TFJ_NewScopeWithDevice(const TFJ_Scope* scope, const char* device);
