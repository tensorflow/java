...
@OpMetadata(
    ...
    inputsClass = FractionalAvgPoolGrad.Inputs.class
)
public final class FractionalAvgPoolGrad<T extends TNumber> extends RawOp implements Operand<T> {
  ...
  public static <T extends TNumber> FractionalAvgPoolGrad<T> create(Scope scope,
    ...
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "FractionalAvgPoolGrad");
  ...
  @OpInputsMetadata(
      outputsClass = FractionalAvgPoolGrad.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<FractionalAvgPoolGrad<T>> {
    /**
     * Original input tensor shape for {@code fractional_avg_pool}
     */
    ...
    public Inputs(GraphOperation op) {
      ...
      int inputIndex = 0;
      origInputTensorShape = (Operand<TInt64>) op.input(inputIndex++);
