 ...
 */
@OpMetadata(
    opType = IteratorGetNext.OP_NAME,
    inputsClass = IteratorGetNext.Inputs.class
)
@Operator(
    group = "data"
)
public final class IteratorGetNext extends RawOp implements Iterable<Operand<TType>> {
  ...
  public IteratorGetNext(Operation operation) {
  ...
  public static IteratorGetNext create(Scope scope, Operand<? extends TType> iterator,
    ...
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "IteratorGetNext");
  ...
  @OpInputsMetadata(
      outputsClass = IteratorGetNext.class
  )
  public static class Inputs extends RawOpInputs<IteratorGetNext> {
    /**
     * The iterator input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new IteratorGetNext(op), op, Arrays.asList("output_types", "output_shapes"));
      int inputIndex = 0;
