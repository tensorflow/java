...
@OpMetadata(
    ...
    inputsClass = FFTND.Inputs.class
)
@Operator
public final class FFTND<T extends TType> extends RawOp implements Operand<T> {
  /**
   * The name of this op, as known by TensorFlow core engine
   */
  public static final String OP_NAME = "FFTND";

  private Output<T> output;

  public FFTND(Operation operation) {
  ...
  public static <T extends TType> FFTND<T> create(Scope scope, Operand<T> input,
      Operand<TInt32> fftLength, Operand<TInt32> axes) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "FFTND");
    opBuilder.addInput(input.asOutput());
    opBuilder.addInput(fftLength.asOutput());
    opBuilder.addInput(axes.asOutput());
    return new FFTND<>(opBuilder.build());
  }

  /**
   * Gets output.
   * A complex tensor of the same shape as {@code input}. The designated
   * dimensions of {@code input} are replaced with their Fourier transforms.
  ...
  @OpInputsMetadata(
      outputsClass = FFTND.class
  )
  public static class Inputs<T extends TType> extends RawOpInputs<FFTND<T>> {
    /**
     * A complex tensor.
     */
    ...
    public Inputs(GraphOperation op) {
      super(new FFTND<>(op), op, Arrays.asList("Tcomplex"));
      int inputIndex = 0;
