 ...
 */
@OpMetadata(
    opType = RepeatDataset.OP_NAME,
    inputsClass = RepeatDataset.Inputs.class
)
@Operator(
    group = "data"
)
public final class RepeatDataset extends RawOp implements Operand<TType> {
  ...
  public RepeatDataset(Operation operation) {
  ...
  public static RepeatDataset create(Scope scope, Operand<? extends TType> inputDataset,
      ...
      Options... options) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "RepeatDataset");
  ...
  /**
   * Optional attributes for {@link org.tensorflow.op.data.RepeatDataset}
   */
  ...
  @OpInputsMetadata(
      outputsClass = RepeatDataset.class
  )
  public static class Inputs extends RawOpInputs<RepeatDataset> {
    /**
    ...
    /**
     * A scalar representing the number of times that {@code input_dataset} should
     * be repeated. A value of {@code -1} indicates that it should be repeated infinitely.
     */
    ...
    public Inputs(GraphOperation op) {
      super(new RepeatDataset(op), op, Arrays.asList("output_types", "output_shapes", "metadata"));
      int inputIndex = 0;
