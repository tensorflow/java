 ...
 */
@OpMetadata(
    opType = BesselI0e.OP_NAME,
    inputsClass = BesselI0e.Inputs.class
)
public final class BesselI0e<T extends TNumber> extends RawOp implements Operand<T> {
  ...
  public BesselI0e(Operation operation) {
  ...
  public static <T extends TNumber> BesselI0e<T> create(Scope scope, Operand<T> x) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "BesselI0e");
    opBuilder.addInput(x.asOutput());
    return new BesselI0e<>(opBuilder.build());
  }
  ...
  @OpInputsMetadata(
      outputsClass = BesselI0e.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<BesselI0e<T>> {
    /**
     * The x input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new BesselI0e<>(op), op, Arrays.asList("T"));
      int inputIndex = 0;
