 ...
 */
@OpMetadata(
    opType = LogicalAnd.OP_NAME,
    inputsClass = LogicalAnd.Inputs.class
)
@Operator(
    group = "math"
)
public final class LogicalAnd extends RawOp implements Operand<TBool> {
  ...
  public LogicalAnd(Operation operation) {
  ...
  public static LogicalAnd create(Scope scope, Operand<TBool> x, Operand<TBool> y) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "LogicalAnd");
  ...
  @OpInputsMetadata(
      outputsClass = LogicalAnd.class
  )
  public static class Inputs extends RawOpInputs<LogicalAnd> {
    /**
     * The x input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new LogicalAnd(op), op, Arrays.asList());
      int inputIndex = 0;
