 ...
 */
@OpMetadata(
    opType = Xlogy.OP_NAME,
    inputsClass = Xlogy.Inputs.class
)
@Operator(
    group = "math"
)
public final class Xlogy<T extends TType> extends RawOp implements Operand<T> {
  ...
  public Xlogy(Operation operation) {
  ...
  public static <T extends TType> Xlogy<T> create(Scope scope, Operand<T> x, Operand<T> y) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "Xlogy");
    opBuilder.addInput(x.asOutput());
    opBuilder.addInput(y.asOutput());
    return new Xlogy<>(opBuilder.build());
  }
  ...
  @OpInputsMetadata(
      outputsClass = Xlogy.class
  )
  public static class Inputs<T extends TType> extends RawOpInputs<Xlogy<T>> {
    /**
     * The x input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new Xlogy<>(op), op, Arrays.asList("T"));
      int inputIndex = 0;
