 ...
 */
@OpMetadata(
    opType = BesselI0.OP_NAME,
    inputsClass = BesselI0.Inputs.class
)
public final class BesselI0<T extends TNumber> extends RawOp implements Operand<T> {
  ...
  public BesselI0(Operation operation) {
  ...
  public static <T extends TNumber> BesselI0<T> create(Scope scope, Operand<T> x) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "BesselI0");
  ...
  @OpInputsMetadata(
      outputsClass = BesselI0.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<BesselI0<T>> {
    /**
     * The x input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new BesselI0<>(op), op, Arrays.asList("T"));
      int inputIndex = 0;
