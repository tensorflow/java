...
public abstract class StridedSliceHelper {
  ...
  static StridedSliceArgs mergeIndexes(Index[] indices) {
    ...
    for (int i = 0; i < indices.length; i++) {
      ...
      if (end[i] != idx.end()) {
        throw new IllegalArgumentException(
            "Can't convert long end value to int for index " + idx + ": Out of bounds");
      }
  ...
  public static <T extends TType> StridedSlice<T> stridedSlice(
      Scope scope, Operand<T> input, Index... indices) {
  ...
  public static <T extends TType> StridedSliceAssign<T> stridedSliceAssign(
      Scope scope, Operand<T> ref, Operand<T> value, Index... indices) {
    StridedSliceArgs args = mergeIndexes(indices);
