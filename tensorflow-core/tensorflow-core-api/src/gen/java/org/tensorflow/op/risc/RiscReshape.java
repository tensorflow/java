 ...
 */
@OpMetadata(
    opType = RiscReshape.OP_NAME,
    inputsClass = RiscReshape.Inputs.class
)
public final class RiscReshape<T extends TNumber> extends RawOp implements Operand<T> {
  ...
  public RiscReshape(Operation operation) {
  ...
  @OpInputsMetadata(
      outputsClass = RiscReshape.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<RiscReshape<T>> {
    /**
     * The tensor input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new RiscReshape<>(op), op, Arrays.asList("T", "Tshape"));
      int inputIndex = 0;
