...
public class XlaOps(
  ...
  ): BroadcastHelper<T> = java.broadcastHelper<T, U>(lhs, rhs, broadcastDims)

  public fun <T : TType> clusterOutput(input: Operand<T>): ClusterOutput<T> =
      java.clusterOutput<T>(input)
  ...
  ): Conv<T> = java.conv<T, U>(lhs, rhs, windowStrides, padding, lhsDilation, rhsDilation,
      featureGroupCount, dimensionNumbers, precisionConfig)
  ...
  ): Dequantize = java.dequantize(input, minRange, maxRange, mode, transposeOutput)
  ...
  public fun <T : TType> dot(
    ...
    dimensionNumbers: String,
    precisionConfig: String
  ): Dot<T> = java.dot<T>(lhs, rhs, dimensionNumbers, precisionConfig)

  public fun <T : TType, U : TNumber> dynamicSlice(
    input: Operand<T>,
    startIndices: Operand<U>,
    sizeIndices: Operand<U>
  ): DynamicSlice<T> = java.dynamicSlice<T, U>(input, startIndices, sizeIndices)

  public fun <T : TType, U : TNumber> dynamicUpdateSlice(
    input: Operand<T>,
    update: Operand<T>,
    indices: Operand<U>
  ): DynamicUpdateSlice<T> = java.dynamicUpdateSlice<T, U>(input, update, indices)

  public fun <T : TType> einsum(
    a: Operand<T>,
    b: Operand<T>,
    equation: String
  ): Einsum<T> = java.einsum<T>(a, b, equation)
  ...
  ): Gather<T> = java.gather<T, U>(operand, startIndices, sliceSizes, dimensionNumbers,
      indicesAreSorted)

  public fun <T : TNumber, U : TType> keyValueSort(keys: Operand<T>, values: Operand<U>):
      KeyValueSort<T, U> = java.keyValueSort<T, U>(keys, values)
  ...
  ): Pad<T> = java.pad<T, U>(input, paddingValue, paddingLow, paddingHigh, paddingInterior)

  public fun <T : TType> recv(
    dtype: DataType<T>,
    tensorName: String,
    shape: Shape
  ): Recv<T> = java.recv<T>(dtype, tensorName, shape)
  ...
  public fun <T : TType> selfAdjointEig(
    ...
    maxIter: Long,
    epsilon: Float
  ): SelfAdjointEig<T> = java.selfAdjointEig<T>(a, lower, maxIter, epsilon)

  public fun <T : TType> send(tensor: Operand<T>, tensorName: String): Send = java.send<T>(tensor,
      tensorName)

  public fun <T : TType> sharding(input: Operand<T>): Sharding<T> = java.sharding<T>(input)

  public fun <T : TType> sort(input: Operand<T>): Sort<T> = java.sort<T>(input)
  ...
  public fun <T : TType> svd(
    ...
    epsilon: Float,
    precisionConfig: String
  ): Svd<T> = java.svd<T>(a, maxIter, epsilon, precisionConfig)
}
