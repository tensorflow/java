 ...
 */
public class Maximum<T extends TFloating> extends Merge<T> {
   ...
   */
  public Maximum(Ops tf, Class<T> type) {
   ...
   */
  public Maximum(Ops tf, Class<T> type, Options options) {
   ...
   * @param type the data type for the weights and computation
   */
  public Maximum(Ops tf, String name, Class<T> type) {
   ...
   * @param options the layer's options.
   */
  public Maximum(Ops tf, String name, Class<T> type, Options options) {
  ...
  protected Operand<? extends TType> mergeFunction(List<Operand<? extends TNumber>> inputs) {
    ...
    for (int i = 1; i < inputs.size(); i++) {
      output = tf.math.maximum(output, cast(tf, inputs.get(i), getType()));
    }
    return output;
  }
}
