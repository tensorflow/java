...
private fun JavadocDescription.preParseTransform(): JavadocDescription {
    val transformedElements = elements.map {
        if (it is JavadocInlineTag && it.type == JavadocInlineTag.Type.CODE)
            it.toText()
        else
            it.toText()
                .replace("\r\n", "\n")
...
internal fun Javadoc.toKDoc(): String = buildString {
    ...
    appendLine()
    appendLine()
    this@toKDoc.blockTags.mapNotNull { it.toKDoc() }.forEach {
        append(it + "\n")
    }
}
...
private fun JavadocBlockTag.toKDoc(): String = when (type) {
    JavadocBlockTag.Type.DEPRECATED -> ""
    JavadocBlockTag.Type.SEE -> directToKDoc { convertRef(it) } //TODO or does this parse as link?
    ...
    JavadocBlockTag.Type.UNKNOWN -> buildString {
...
private fun String.replaceTag(with: String, vararg tags: String) = tags.fold(this) { current, tag ->
    current.replace("<$tag>", with).replace("</$tag>", with)
}
...
private fun JavadocDescription.toKDoc(): String {
    ...
    return preParseTransform().elements.joinToString("") { it.toKDoc() }
        .replace("\r\n", "\n")
        .replace("&lt;", "<")
        .replace("&gt;", ">")
        .replaceTag("\n", "p", "br")
...
private fun JavadocDescriptionElement.toKDoc(): String = if (this is JavadocInlineTag)
    this.toKDoc()
else
    this.toText().replaceProblematicBrackets()

private fun convertRef(ref: String) = ref.substringBefore('(').replace("#", ".")

private fun convertLink(link: String): String = if (" " in link) {
...
private val JavadocInlineTag.trimmedContent get() = content.trimStart()

private fun makeCodeBlock(content: String): String {
    val stripedContent = if (content.startsWith("{@code "))
        content.removePrefix("{@code ").removeSuffix("}")
    else
        content

    val isMultiline = stripedContent.lines().size > 1

    val escapedContent = if (isMultiline)
        stripedContent
    else
        stripedContent.replaceProblematicBrackets()

    return if (isMultiline) "```\n$escapedContent\n```" else "`$escapedContent`"
}

internal fun JavadocInlineTag.toKDoc(): String = when (type) {
