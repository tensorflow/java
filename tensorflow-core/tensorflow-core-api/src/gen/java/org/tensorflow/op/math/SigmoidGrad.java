...
@OpMetadata(
    ...
    inputsClass = SigmoidGrad.Inputs.class
)
public final class SigmoidGrad<T extends TType> extends RawOp implements Operand<T> {
  /**
  ...
  public SigmoidGrad(Operation operation) {
  ...
  public static <T extends TType> SigmoidGrad<T> create(Scope scope, Operand<T> y, Operand<T> dy) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "SigmoidGrad");
    opBuilder.addInput(y.asOutput());
    opBuilder.addInput(dy.asOutput());
    return new SigmoidGrad<>(opBuilder.build());
  }
  ...
  @OpInputsMetadata(
      outputsClass = SigmoidGrad.class
  )
  public static class Inputs<T extends TType> extends RawOpInputs<SigmoidGrad<T>> {
    /**
     * The y input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new SigmoidGrad<>(op), op, Arrays.asList("T"));
      int inputIndex = 0;
