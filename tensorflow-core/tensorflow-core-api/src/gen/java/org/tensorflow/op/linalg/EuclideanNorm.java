 ...
 */
@OpMetadata(
    opType = EuclideanNorm.OP_NAME,
    inputsClass = EuclideanNorm.Inputs.class
)
@Operator(
    group = "linalg"
)
public final class EuclideanNorm<T extends TType> extends RawOp implements Operand<T> {
  ...
  public EuclideanNorm(Operation operation) {
  ...
  public static <T extends TType> EuclideanNorm<T> create(Scope scope, Operand<T> input,
      Operand<? extends TNumber> axis, Options... options) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "EuclideanNorm");
    ...
    return new EuclideanNorm<>(opBuilder.build());
  }
  ...
  @OpInputsMetadata(
      outputsClass = EuclideanNorm.class
  )
  public static class Inputs<T extends TType> extends RawOpInputs<EuclideanNorm<T>> {
    /**
    ...
    public Inputs(GraphOperation op) {
      super(new EuclideanNorm<>(op), op, Arrays.asList("keep_dims", "T", "Tidx"));
      int inputIndex = 0;
