...
public abstract class BooleanMask {
  ...
  public static <T extends TType> Operand<T> create(
      Scope scope, Operand<T> tensor, Operand<TBool> mask, Options... options) {
    ...
    org.tensorflow.op.core.Shape<TInt32> liveShape =
        org.tensorflow.op.core.Shape.create(scope, tensor);

    Operand<TInt32> leadingSize =
        ReduceProd.create(
            scope,
            StridedSliceHelper.stridedSlice(
                scope, liveShape, Indices.range(axis, axis + maskShape.numDimensions())),
    ...
    Operand<TInt64> indices =
        Squeeze.create(
            scope, Where.create(scope, flatMask), Squeeze.axis(Collections.singletonList(1L)));
