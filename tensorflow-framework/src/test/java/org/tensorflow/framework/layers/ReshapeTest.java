...
class ReshapeTest {
  ...
  public void testCall43() {
    try (TestSession session = TestSession.createTestSession(tfMode)) {
      ...
      long batchSize = 2;
      Reshape<TFloat32> instance =
          new Reshape<>(
              tf,
              targetShape,
              TFloat32.class,
              Layer.Options.create().inputShape(Shape.of(batchSize, 3, 4)));
    ...
    try (TestSession session = TestSession.createTestSession(tfMode)) {
      ...
      Reshape<TFloat32> instance =
          new Reshape<>(
              ...
              targetShape,
              TFloat32.class,
              Layer.Options.create()
                  .inputShape(Shape.of(Shape.UNKNOWN_SIZE, Shape.UNKNOWN_SIZE, 2)));

      Operand<TFloat64> result =
          instance.call(tf.dtypes.cast(tf.constant(inputArrayNN2), TFloat64.class), TFloat64.class);
