...
public class VariableDef<T extends TNumber> {
  ...
  /**
   * Creates a VariableDef
   *
   * @param tf the TensorFlow Ops
   * @param name the name of the variable
   * @param shape the shpe of the variable
   * @param initializer the initializer for the variable
   * @param constraint the constraint for the variable
   * @param regularizer the regularizer for the variable
   * @param trainable inidcator that this variable is trainable or not
   * @param seed a seed for random number generation.
   * @param type the data typt of the variable.
   */
  public VariableDef(
      Ops tf,
      String name,
    ...
    initOperand = this.initializer.call(tf, tf.constant(this.shape), type);
  ...
  private Initializer<T> getDefaultInitializer(long seed) {
    ...
    if (TFloating.class.isAssignableFrom(type)) {
      ...
      initializer = (Initializer<T>) new Glorot<>(VarianceScaling.Distribution.UNIFORM, seed);
    } else {
