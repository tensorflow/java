...
public class ImageOps(
     ...
     *  ``` [0, image_height - 1]``` in image height coordinates. We do allow ``` y1``` &gt; ```
     * y2```, in
     ...
     *  ``` "bilinear"``` or ``` "nearest"``` and default to ``` "bilinear"```. Currently two
     * sampling
     ...
     *  ``` y``` is mapped to the image coordinate at ``` y * (image_height - 1)```, so as the
     *  ``` [0, 1]``` interval of normalized image height is mapped to
     *  ``` [0, image_height - 1] in image height coordinates. We do allow y1 > y2, in which case
     * the sampled crop is an up-down flipped version of the original image. The width dimension is
     * treated similarly. Normalized coordinates outside the ```&#91;0, 1]``` range are allowed, in
     ...
     *  dtype.
     *  <em>NOTE</em>: decode_gif returns a 4-D array &#91;num_frames, height, width, 3], as
     ...
     *  <em>NOTE</em>: If the first frame of an animated GIF does not occupy the entire
     ...
     *  content, i.e. <em>data augmentation</em>. This Op, given the same ``` seed```,
