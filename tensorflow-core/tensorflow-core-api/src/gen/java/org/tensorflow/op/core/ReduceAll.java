 ...
 */
@OpMetadata(
    opType = ReduceAll.OP_NAME,
    inputsClass = ReduceAll.Inputs.class
)
@Operator
public final class ReduceAll extends RawOp implements Operand<TBool> {
  ...
  public ReduceAll(Operation operation) {
  ...
  public static ReduceAll create(Scope scope, Operand<TBool> input, Operand<? extends TNumber> axis,
      Options... options) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "ReduceAll");
  ...
  @OpInputsMetadata(
      outputsClass = ReduceAll.class
  )
  public static class Inputs extends RawOpInputs<ReduceAll> {
    /**
    ...
    public Inputs(GraphOperation op) {
      super(new ReduceAll(op), op, Arrays.asList("keep_dims", "Tidx"));
      int inputIndex = 0;
