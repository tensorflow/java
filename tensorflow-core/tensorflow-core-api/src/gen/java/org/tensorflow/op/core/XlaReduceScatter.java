 ...
 */
@Operator
public final class XlaReduceScatter<T extends TNumber> extends RawOp implements Operand<T> {
  /**
  ...
  private XlaReduceScatter(Operation operation) {
    super(operation);
    int outputIdx = 0;
  ...
  public static <T extends TNumber> XlaReduceScatter<T> create(Scope scope, Operand<T> input,
      Operand<TInt32> groupAssignment, Operand<TInt32> scatterDimension, String reduceOp) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "XlaReduceScatter");
    ...
    return new XlaReduceScatter<>(opBuilder.build());
  }
  ...
  public static class Inputs<T extends TNumber> extends RawOpInputs<XlaReduceScatter<T>> {
    /**
    ...
    public Inputs(GraphOperation op) {
      super(new XlaReduceScatter<>(op), op, Arrays.asList("T", "reduce_op"));
      int inputIndex = 0;
