...
@OpMetadata(
    ...
    inputsClass = Minimum.Inputs.class
)
@Operator(
    group = "math"
)
public final class Minimum<T extends TNumber> extends RawOp implements Operand<T> {
  /**
   * The name of this op, as known by TensorFlow core engine
   */
  public static final String OP_NAME = "Minimum";

  private Output<T> z;

  public Minimum(Operation operation) {
  ...
  public static <T extends TNumber> Minimum<T> create(Scope scope, Operand<T> x, Operand<T> y) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "Minimum");
    opBuilder.addInput(x.asOutput());
    opBuilder.addInput(y.asOutput());
    return new Minimum<>(opBuilder.build());
  }
  ...
  @OpInputsMetadata(
      outputsClass = Minimum.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<Minimum<T>> {
    /**
     * The x input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new Minimum<>(op), op, Arrays.asList("T"));
      int inputIndex = 0;
