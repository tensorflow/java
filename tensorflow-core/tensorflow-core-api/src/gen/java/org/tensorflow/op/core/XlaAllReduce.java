 ...
 */
@Operator
public final class XlaAllReduce<T extends TNumber> extends RawOp implements Operand<T> {
  /**
  ...
  private XlaAllReduce(Operation operation) {
    super(operation);
    int outputIdx = 0;
  ...
  public static <T extends TNumber> XlaAllReduce<T> create(Scope scope, Operand<T> input,
      Operand<TInt32> groupAssignment, String reduceOp) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "XlaAllReduce");
    opBuilder.addInput(input.asOutput());
    opBuilder.addInput(groupAssignment.asOutput());
    opBuilder.setAttr("reduce_op", reduceOp);
    return new XlaAllReduce<>(opBuilder.build());
  }
  ...
  public static class Inputs<T extends TNumber> extends RawOpInputs<XlaAllReduce<T>> {
    /**
    ...
    public Inputs(GraphOperation op) {
      super(new XlaAllReduce<>(op), op, Arrays.asList("T", "reduce_op"));
      int inputIndex = 0;
