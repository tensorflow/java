...
/**
 * Data layout that converts 32-bit floats from/to 16-bit, accordingly to the IEEE-754
 * half-precision floating point specification.
 */
public final class Float16Layout implements FloatDataLayout<ShortDataBuffer> {
  ...
  static short float32to16(float f32) {
    ...
    if (i32 >= (E32BIAS + E16MAX + 1) << E32SHIFT) {
      ...
      // - apply magic number to align significand 10 bits at the bottom on the float and subtract
      // bias
      i16 = Float.floatToIntBits(Float.intBitsToFloat(i32) + MAGIC_32_16_FLOAT) - MAGIC_32_16;

    } else {
      // float32 value can be rounded up to a normalized float16 value (i.e. exp32 = [113(-14),
      // 142(15)])
  ...
  private static final int MAGIC_32_16 =
      ((E32BIAS - E16BIAS) + (S32BITS - S16BITS) + 1) << E32SHIFT;
