...
public abstract class BooleanMaskUpdate {
  ...
  public static <T extends TType> Operand<T> create(
      Scope scope, Operand<T> tensor, Operand<TBool> mask, Operand<T> updates, Options... options) {
    ...
    if (!requiredMaskShape.isCompatibleWith(maskShape)) {
      throw new IllegalArgumentException(
          "Mask shape "
              + maskShape
              + " is not compatible with the required mask shape: "
              + requiredMaskShape
              + ".");
    }
    ...
    Operand<TInt32> leadingSize =
        ReduceProd.create(
            scope,
            StridedSliceHelper.stridedSlice(
                scope, liveShape, Indices.sliceTo(axis + maskShape.numDimensions())),
            Constant.arrayOf(scope, 0));

    Operand<TInt32> innerShape =
        StridedSliceHelper.stridedSlice(
            scope, liveShape, Indices.sliceFrom(axis + maskShape.numDimensions()));
    ...
    Operand<T> reshaped =
        Reshape.create(
            ...
            Concat.create(
                scope,
                Arrays.asList(
                    Reshape.create(scope, leadingSize, Constant.arrayOf(scope, 1)), innerShape),
    ...
    if (broadcast) {
      ...
      Operand<TInt32> batchShape =
          StridedSliceHelper.stridedSlice(scope, indicesShape, Indices.sliceTo(-1));

      Operand<TInt32> updateShape =
          Concat.create(scope, Arrays.asList(batchShape, innerShape), Constant.scalarOf(scope, 0));
  ...
  /**
   * Used to indicate the axis to mask from. {@code axis + dim(mask) <= dim(tensor)} and {@code
   * mask}'s shape must match the first {@code axis + dim(mask)} dimensions of {@code tensor}'s
   * shape.
   *
