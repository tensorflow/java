 ...
 */
public final class XlaConcatND<T extends TType> extends RawOp implements Operand<T> {
  /**
  ...
  private XlaConcatND(Operation operation) {
    super(operation);
    int outputIdx = 0;
   ...
   * @return a new instance of XlaConcatND
   */
  @Endpoint(
      describeByClass = true
  )
  public static <T extends TType> XlaConcatND<T> create(Scope scope, Iterable<Operand<T>> inputs,
      List<Long> numConcats, Options... options) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "XlaConcatND");
    ...
    return new XlaConcatND<>(opBuilder.build());
  }
  ...
  /**
   * Optional attributes for {@link org.tensorflow.op.rawops.XlaConcatND}
   */
  ...
  public static class Inputs<T extends TType> extends RawOpInputs<XlaConcatND<T>> {
    /**
    ...
    public Inputs(GraphOperation op) {
      super(new XlaConcatND<>(op), op, Arrays.asList("T", "num_concats", "paddings"));
      int inputIndex = 0;
