 ...
 */
@OpMetadata(
    opType = LessEqual.OP_NAME,
    inputsClass = LessEqual.Inputs.class
)
@Operator(
    group = "math"
)
public final class LessEqual extends RawOp implements Operand<TBool> {
  ...
  public LessEqual(Operation operation) {
  ...
  public static <T extends TNumber> LessEqual create(Scope scope, Operand<T> x, Operand<T> y) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "LessEqual");
  ...
  @OpInputsMetadata(
      outputsClass = LessEqual.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<LessEqual> {
    /**
     * The x input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new LessEqual(op), op, Arrays.asList("T"));
      int inputIndex = 0;
