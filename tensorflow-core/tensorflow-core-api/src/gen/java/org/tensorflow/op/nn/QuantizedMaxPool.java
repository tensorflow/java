...
@OpMetadata(
    ...
    inputsClass = QuantizedMaxPool.Inputs.class
)
@Operator(
    group = "nn"
)
public final class QuantizedMaxPool<T extends TNumber> extends RawOp {
  /**
  ...
  public static <T extends TNumber> QuantizedMaxPool<T> create(Scope scope, Operand<T> input,
      ...
      String padding) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "QuantizedMaxPool");
    ...
    return new QuantizedMaxPool<>(opBuilder.build());
  }
  ...
  @OpInputsMetadata(
      outputsClass = QuantizedMaxPool.class
  )
  public static class Inputs<T extends TNumber> extends RawOpInputs<QuantizedMaxPool<T>> {
    /**
