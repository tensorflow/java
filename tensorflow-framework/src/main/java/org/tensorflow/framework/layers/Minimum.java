 ...
 */
public class Minimum<T extends TFloating> extends Merge<T> {
   ...
   */
  public Minimum(Ops tf, Class<T> type) {
   ...
   * @param type the data type for the weights and computation
   */
  public Minimum(Ops tf, Class<T> type, Options options) {
   ...
   * @param type the data type for the weights and computation
   */
  public Minimum(Ops tf, String name, Class<T> type) {
   ...
   * @param type the data type for the weights and computation
   */
  public Minimum(Ops tf, String name, Class<T> type, Options options) {
  ...
  protected Operand<? extends TType> mergeFunction(List<Operand<? extends TNumber>> inputs) {
    ...
    for (int i = 1; i < inputs.size(); i++) {
      output = tf.math.minimum(output, cast(tf, inputs.get(i), getType()));
    }
    return output;
  }
}
