  ...
  public void testSequentialLayers() {
    ...
    try (TestSession session = TestSession.createTestSession(tfMode)) {
      ...
      List<Layer<TFloat32>> sequencedLayers =
          Arrays.asList(
              new Input<>(tf, input, TFloat32.class),
              new Dense<>(tf, 3, seed, TFloat32.class),
              new Dropout<>(tf, 0.3f, seed, TFloat32.class),
              new Flatten<>(tf, TFloat32.class));

      Operand<TFloat32> result = input;
      for (Layer<TFloat32> layer : sequencedLayers) {
        result = layer.call(result, TFloat32.class);
      }
      session.run(tf.init());
      float[][] expected =
          new float[][] {
