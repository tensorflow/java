...
public final class Div<T extends TType> extends RawOp implements Operand<T> {
  ...
  public Div(Operation operation) {
  ...
  public static <T extends TType> Div<T> create(Scope scope, Operand<T> x, Operand<T> y) {
    OperationBuilder opBuilder = scope.opBuilder(OP_NAME, "Div");
  ...
  @OpInputsMetadata(
      outputsClass = Div.class
  )
  public static class Inputs<T extends TType> extends RawOpInputs<Div<T>> {
    /**
     * The x input
     */
    ...
    public Inputs(GraphOperation op) {
      super(new Div<>(op), op, Arrays.asList("T"));
      int inputIndex = 0;
